...
public class GetCollectInfoPresenter implements Presenter {
    private final String TAG = GetCollectInfoPresenter.class.getSimpleName();
    ...
    public GetCollectInfoPresenter(Context mContext) {
     ...
     * @param requestParams
     */
    public void getCollectInfo(CollectionRequestParams requestParams) {
        RequestBody body = RequestBody.create(okhttp3.MediaType.parse(
                Strings.MEDIATYPE_JSON), new Gson().toJson(requestParams));

        mCompositeSubscription.add(manager.getCollectInfo(body)
