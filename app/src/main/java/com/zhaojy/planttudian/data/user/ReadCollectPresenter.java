...
public class ReadCollectPresenter implements Presenter {
    private final String TAG = ReadCollectPresenter.class.getSimpleName();
    ...
    public ReadCollectPresenter(Context mContext) {
     ...
     * 读取收藏
     */
    public synchronized void readCollect() {
        if (isLoading) {
            return;
        }
        isLoading = true;
        ...
        mCompositeSubscription.add(manager.readCollect(body)
                ...
                .subscribe(new Observer<List<Collect>>() {
                    ...
                    public void onCompleted() {
                        if (collectList != null) {
                            update.onSuccess(collectList);
                            //增加偏移量
                            offset += collectList.size();
                        }
